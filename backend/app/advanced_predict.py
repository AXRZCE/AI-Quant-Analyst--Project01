...
def get_prediction_pipeline():
    """Get the prediction pipeline or raise an exception if not available."""
    if not ADVANCED_MODELS_AVAILABLE:
        logger.error("Advanced models not available")
        raise ValueError("Advanced prediction models are not available")
    ...
    try:
        ...
        raise ValueError(f"Failed to create prediction pipeline: {e}")


def get_sentiment_analyzer():
    """Get the sentiment analyzer or raise an exception if not available."""
    if not ADVANCED_MODELS_AVAILABLE:
        logger.error("Advanced models not available")
        raise ValueError("Sentiment analysis models are not available")

    try:
        from src.nlp.finbert_sentiment import FinBERTSentiment
        return FinBERTSentiment()
    except (ImportError, Exception) as e:
        logger.error(f"FinBERT sentiment analyzer not available: {e}")
        raise ValueError(f"Failed to load sentiment analyzer: {e}")


def get_model_integrator():
    """Get the model integrator or raise an exception if not available."""
    if not ADVANCED_MODELS_AVAILABLE:
        logger.error("Advanced models not available")
        raise ValueError("Model integrator is not available")

    try:
        from src.models.advanced_models import ModelIntegrator
        return ModelIntegrator()
    except (ImportError, Exception) as e:
        logger.error(f"Model integrator not available: {e}")
        raise ValueError(f"Failed to load model integrator: {e}")
...
def analyze_sentiment(news: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    try:
        ...
        if not texts:
            ...
            raise ValueError("No news texts available for sentiment analysis")
...
def run_advanced_prediction(symbol: str, days: int = 30) -> Dict[str, Any]:
    ...
    Raises:
        ValueError: If prediction cannot be made with real data
    """
