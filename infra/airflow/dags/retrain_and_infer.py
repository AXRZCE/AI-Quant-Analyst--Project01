...
with DAG(
    ...
    run_drift_detection = BashOperator(
        ...
        bash_command=(
            ...
            "--output-path {{ var.value.project_dir }}/reports/drift_$(date +\\%Y-\\%m-\\%d).html"
        ),
    )
    
    # Define task dependencies
    ingest_data >> generate_features >> train_baseline >> save_to_bentoml >> build_bento >> containerize_bento >> deploy_to_k8s >> check_service_health >> run_drift_detection
